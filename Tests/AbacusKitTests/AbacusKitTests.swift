...
final class AbacusKitTests: XCTestCase {
    ...
    func testInterpreterMultipleDigits() {
        ...
        // 123 を表現
        let digit1 = SorobanDigit(position: 2, upperBead: .upper, lowerBeads: [.lower, .upper, .upper, .upper], confidence: 1.0, boundingBox: .zero)
        let digit2 = SorobanDigit(position: 1, upperBead: .upper, lowerBeads: [.lower, .lower, .upper, .upper], confidence: 1.0, boundingBox: .zero)
        let digit3 = SorobanDigit(position: 0, upperBead: .upper, lowerBeads: [.lower, .lower, .lower, .upper], confidence: 1.0, boundingBox: .zero)
